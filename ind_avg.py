...
genselectedFeatures = ["bikes_3h_ago",
"short_profile_bikes",
"short_profile_3h_diff_bikes",
"full_profile_bikes",
"full_profile_3h_diff_bikes",
"temperature.C"]
gentraining_feature_cols = ()
...
for x in genselectedFeatures:
    trainIdx = train_feat.index(x)
    gentraining_feature_cols = training_feature_cols + (trainIdx,)
...
difs = np.genfromtxt("difs.csv", dtype=float, comments='#', delimiter=',',
...
# Read in the training features and target variable
    training_features = np.genfromtxt(filestring, dtype=float, comments='#', delimiter=',',
        ...
        autostrip=False, case_sensitive=True, defaultfmt='f%i',
        unpack=None, usemask=False, loose=True, invalid_raise=True)
    gentraining_features = np.genfromtxt(filestring, dtype=float, comments='#', delimiter=',',
        skip_header=1, skip_footer=0, converters=None, missing_values={"NA"},
        filling_values=0, usecols=gentraining_feature_cols,
        names=None, excludelist=None, deletechars=None, replace_space='_',
        autostrip=False, case_sensitive=True, defaultfmt='f%i',
        unpack=None, usemask=False, loose=True, invalid_raise=True)
...
# Generate a model for this particular station
    ...
    genclf = linear_model.LinearRegression(fit_intercept=True, normalize=False, copy_X=True, n_jobs=1)
    ...
    genclf.fit (gentraining_features, training_target)
    #print 'Coefficients: \n\t', clf.coef_
    genclf.intercept_ = 0.30381645061
    genclf.coef_[0] = 0.744290931563
    genclf.coef_[1] = 0.0643842157692 
    genclf.coef_[2] = 0.223470461553
    genclf.coef_[3] = 0.16065454214
    genclf.coef_[4] = 0.36816804663
    genclf.coef_[5] = -0.00470908330151
    predictions.extend(clf.predict(station_test_features).tolist())
    genpredictions.extend(genclf.predict(station_test_features).tolist())

    # coefout.write(str(clf.intercept_)+ ',')   
    # for coef in clf.coef_:
    #     coefout.write(str(coef) + ",")
    # coefout.write('\n')
for p in range(0, len(predictions)):
    predictions[p] = np.mean([predictions[p],genpredictions[p]])
for p in range(0, len(predictions)):
...
if mock:
    ...
    print 'DIFF ', mean_absolute_error(truevalues,baseline) - mean_absolute_error(truevalues,predictions)
